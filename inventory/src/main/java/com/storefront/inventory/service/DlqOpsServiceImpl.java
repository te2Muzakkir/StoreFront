...
import com.storefront.inventory.repository.InventoryDlqRepository;
...
public class DlqOpsServiceImpl implements DlqOpsService {
	...
	private final InventoryDlqRepository inventoryDlqRepository;
	...
	public void retryFromDlq(Long id) {
		try {
			InventoryDlq dlq = inventoryDlqRepository.findById(id).orElseThrow();
			if ("INVENTORY".equals(dlq.getAction())) {
			...
			if ("PAYMENT".equals(dlq.getAction())) {
			...
			inventoryDlqRepository.save(dlq);
		} catch (Exception e) {
    ...
    public void saveInventoryFailure(Message<InventoryCommand> message) {
        ...
        try {
            ...
            inventoryDlqRepository.save(new InventoryDlq(command.eventId(), command.orderId(),
                            command.action().toString(), payload, exceptionMessage));
