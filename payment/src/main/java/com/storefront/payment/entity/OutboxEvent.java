...
public class OutboxEvent {
    ...
    public OutboxEvent(String aggregateType, String aggregateId,
        ...
        this.status = PaymentConstants.OUTBOX_STATUS_PENDING;
        this.retryCount = 0;
        this.createdAt = LocalDateTime.now();
    }
    
    public void markProcessing() {
        if (!PaymentConstants.OUTBOX_STATUS_PENDING.equals(this.status)) 
            throw new IllegalStateException("Only PENDING events can be processed");
        this.status = PaymentConstants.OUTBOX_STATUS_PROCESSING;
        this.processingStartedAt = LocalDateTime.now();
    }

    public void markPublished() {
        this.status = PaymentConstants.OUTBOX_STATUS_PUBLISHED;
    ...
    public void markFailed(String error, int maxRetries) {
        ...
        if (this.retryCount >= maxRetries) 
            this.status = PaymentConstants.OUTBOX_STATUS_FAILED;
        else
        	this.status = PaymentConstants.OUTBOX_STATUS_PENDING;
    }
